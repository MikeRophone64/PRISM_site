<?php
/**
 * Template Name: PRISM Booking Page — EN
 *
 * ─── SETUP ───────────────────────────────────────────────────────────────────
...
<title>Book a Strategic Diagnostic — PRISM Filmmaking</title>
<?php wp_head(); ?>
...
@media (max-width: 600px) {
  /* Nav: tighten up, prevent overflow */
  nav {
  ...
  /* Widget: reduce padding so content has room */
  .booking-widget { padding: 1.5rem 1rem; }

  /* Step indicators: smaller and tighter */
  ...
  /* Calendar: smaller cells, no aspect-ratio lock */
...
<div class="booking-page">
  ...
  <div class="booking-inner">
    ...
    <div class="booking-info">
      <p class="section-label">Strategic Diagnostic</p>
      <h1 class="booking-title">Book your<br><span>30 minutes.</span></h1>
      <p class="booking-desc">
        Not a sales call. A focused conversation to find out whether and how PRISM can make your expertise impossible to ignore.
      </p>
      <div class="booking-meta">
        <div class="meta-item">
          <div class="meta-icon">⏱</div>
          <div><strong>30 minutes</strong>Focused, no fluff</div>
        </div>
        <div class="meta-item">
          <div class="meta-icon">🎥</div>
          <div><strong>Google Meet</strong>Link sent on confirmation</div>
        </div>
        <div class="meta-item">
          <div class="meta-icon">✦</div>
          <div><strong>Free diagnostic</strong>You leave with actionable insights</div>
        </div>
        <div class="meta-item">
          <div class="meta-icon">◈</div>
          <div><strong>3 projects per quarter</strong>Availability is genuinely limited</div>
        </div>
      </div>
    </div>
    ...
    <div class="booking-widget">
      ...
      <div id="step-2">
        ...
        <button class="btn-back" id="btn-back-1">← Change date &amp; time</button>
        <div class="form-row">
        ...
        <div class="form-group">
          <label class="form-label" for="f-website">Company website *</label>
          <input class="form-input" type="url" id="f-website" placeholder="https://yourcompany.com" required>
        </div>
        <div class="form-group">
          <label class="form-label" for="f-message">What would you like us to know?</label>
          <textarea class="form-textarea" id="f-message" placeholder="Tell us about your company, your goals, or anything relevant..."></textarea>
        </div>
      ...
      <div id="step-3">
        ...
        <p class="confirm-subtitle">A calendar invitation and confirmation email are on their way to you.</p>
        <div class="confirm-detail">
          <div class="confirm-detail-row"><strong>Date</strong><span id="conf-date"></span></div>
          <div class="confirm-detail-row"><strong>Time</strong><span id="conf-time"></span></div>
          <div class="confirm-detail-row"><strong>Duration</strong>30 minutes</div>
          <div class="confirm-detail-row"><strong>Format</strong>Google Meet</div>
        </div>
        <a href="#" id="conf-meet-link" class="confirm-meet" style="display:none;">Join Google Meet →</a>
        <a href="<?php echo esc_url($url_en); ?>" class="confirm-back">← Back to PRISM</a>
      </div>
...
async function selectDate(dateStr) {
  ...
  container.innerHTML = '<div class="slots-loading">Loading availability</div>';
  ...
  try {
    ...
    if (!state.slots.length) {
      container.innerHTML = '<div class="slots-empty">No availability on this day — please pick another date.</div>';
      return;
    }
    ...
    container.innerHTML = '<div class="slots-empty">Could not load availability. Please try again.</div>';
  }
}
...
<style>
footer {
  background: #050505; padding: 3rem 3rem 2rem;
  border-top: 1px solid rgba(252,252,252,0.05);
  position: relative; overflow: hidden;
}
footer::before {
  content: ''; position: absolute; top: 0; left: 0; right: 0; height: 1px;
  background: linear-gradient(90deg, transparent, var(--c-prism-a) 25%, var(--c-prism-b) 50%, var(--c-orange) 75%, transparent);
  opacity: 0.3;
}
.footer-row1 { display: grid; grid-template-columns: 0.6fr 1fr 1fr; gap: 2rem; align-items: start; margin-bottom: 2.5rem; }
.footer-mission { font-size: 0.73rem; font-weight: 300; color: rgba(252,252,252,0.28); line-height: 1.7; }
.footer-legal { display: flex; flex-direction: column; gap: 0.55rem; align-items: center; }
.footer-legal a { font-size: 0.63rem; letter-spacing: 0.1em; text-transform: uppercase; color: rgba(252,252,252,0.22); text-decoration: none; transition: color 0.2s; }
.footer-legal a:hover { color: rgba(252,252,252,0.6); }
.footer-social { display: flex; gap: 1.25rem; justify-content: flex-end; align-items: center; }
.footer-social a { color: rgba(252,252,252,0.28); transition: color 0.3s, transform 0.3s; display: flex; }
.footer-social a:hover { color: var(--c-prism-a); transform: scale(1.15); }
.footer-social svg { width: 20px; height: 20px; fill: currentColor; }
.footer-row2 { text-align: center; font-size: 0.62rem; letter-spacing: 0.14em; text-transform: uppercase; color: rgba(252,252,252,0.13); border-top: 1px solid rgba(252,252,252,0.04); padding-top: 1.5rem; }
@media (max-width: 900px) {
...
<footer>
  <div class="footer-row1">
    ...
    <div class="footer-legal">
      ...
      <a href="https://prism.film/politique-de-cookies-ue">Cookie Policy</a>
      <a href="https://prism.film/privacy-policy">Privacy Policy</a>
    </div>
  ...
  <div class="footer-row2">© <?php echo $site_year; ?> PRISM Filmmaking. All rights reserved.</div>
</footer>
